...
<template>
    <Head :title="lang[locale].eshopBasketTitle"/>
    <main-layout>
        <div class="my-4 max-w-xl mx-auto">
            ...
            <section v-if="!basket.length" class="">
                ...
                <Link :href="route('books', ['vsetko'])"
                      class="block rounded text-gray-500 text-center text-white px-4 py-3 my-4 shadow-md bg-pink-500 hover:bg-pink-600">
                    <ShoppingBagIcon class="mb-1 w-5 h-5 inline"/>
                    {{ lang[locale].eshopBasketEmptyButton }}
                </Link>
            </section>

            <section v-if="basket.length">
                <div v-for="book in basket" :key="book.id"
                     class="grid grid-cols-12 gap-4 py-4 border-b border-gray-300">
                    <div class="col-span-4 sm:col-span-2">
                        <img :alt="book.title"
                             :src="`/storage/${book.cover}`" class="w-32 sm:w-20 h-auto shadow border border-gray-200 rounded">
                    </div>
            ...
            <section v-if="basket.length"
                     class="my-6 flex flex-col sm:flex-row flex-col-reverse justify-between items-center text-sm sm:text-base">
                <Link :href="route('books', ['vsetko'])"
                      class="rounded text-gray-500 text-center px-4 py-3 bg-gray-200 hover:bg-gray-300">
                    <ShoppingBagIcon class="mb-1 w-5 h-5 inline"/>
                    {{ lang[locale].eshopBackButtonBasket }}
                </Link>
                <Link
                    :href="route('shipping')"
